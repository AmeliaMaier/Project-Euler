...
def prod_diagonally_down_right(max, list_ind, num_ind, number_grid):
    prod = 1
    for numbers in number_grid[list_ind:list_ind+4]:
        prod *= numbers[num_ind]
        num_ind += 1
    if prod > max:
        max = prod
    return max
def prod_diagonally_up_right(max, list_ind, num_ind, number_grid):
    prod = 1
    for numbers in number_grid[list_ind:list_ind-4:-1]:
        prod *= numbers[num_ind]
        num_ind += 1
    if prod > max:
        max = prod
    return max
def prod_right(max, ind, numbers):
    prod = 1
    for num in numbers[ind:ind+4]:
        prod *= num
    if prod > max:
        max = prod
    return max
def prod_down(max, list_ind, num_ind, number_grid):
    prod = 1
    for numbers in number_grid[list_ind:list_ind+4]:
        prod *= numbers[num_ind]
    if prod > max:
...
row2 = [49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 4, 56, 62, 00]
...
number_grid = [row1,row2,row3,row4,row5,row6,row7,row8,row9,row10,row11,row12,row13,row14,row15,row16,row17,row18,row19,row20]

max = 0
for list_ind, numbers in enumerate(number_grid):
    for num_ind, number in enumerate(number_grid):
        if num_ind+3 < len(numbers):
            max = prod_right(max, num_ind, numbers)
        if list_ind+3 <len(number_grid):
            max = prod_down(max, list_ind, num_ind, number_grid)
        if num_ind+3 < len(numbers) and list_ind+3 <len(number_grid):
            max = prod_diagonally_down_right(max, list_ind, num_ind, number_grid)
        if num_ind+3 < len(numbers) and list_ind-3 >=0:
            max = prod_diagonally_up_right(max, list_ind, num_ind, number_grid)
        print(max)
